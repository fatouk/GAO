...
export class ReservationUpdateComponent implements OnInit {
  ...
  constructor(public formBuilder: FormBuilder,private messageService:MessageService,private reservationService:ReservationService,
   ...
   async loadReservation(id:number){
    let data = await this.reservationService.getReservationByid(id).toPromise();
    this.reservation=data;
      this.reservationUpdateForm = new FormGroup({
      utilisateur1!: new FormControl(this.reservation!.utilisateur, [Validators.required,Validators.maxLength(240)]),
      ordinateur1!: new FormControl(this.reservation!.ordinateur,[Validators.required,Validators.maxLength(240)]),
  ...
  return this.reservationUpdateForm.pristine;
  }
  ...
  get utilisateur() {return this.reservationUpdateForm.get('utilisateur1'); }
  get ordinateur() { return this.reservationUpdateForm.get('ordinateur1'); }
  ...
  this.loadReservation(this.id);
    ...
    let result: Utilisateur[]= await this.utilisateurService.getUsers().toPromise();
     for(let q1 of result){
      if(this.reservation.utilisateur && q1.id===this.reservation.utilisateur!.id){
 ...
 async loadOrdinateur(){
  ...
  let result: Ordinateur[]= await this.ordinateurService.getOrdinateurs().toPromise();
   for(let q of result){
    if(this.reservation.ordinateur && q.id===this.reservation.ordinateur!.id){
 ...
 async updateReservation(id:number,reservation:Reservation){
  if (this.reservationUpdateForm.invalid) {
   ...
   try {
      reservation.utilisateur=this.reservationUpdateForm.get('utilisateur1')!.value;
      reservation.ordinateur=this.reservationUpdateForm.get('ordinateur1')!.value;
